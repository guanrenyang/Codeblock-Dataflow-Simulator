...
class LoadSignalPackage final : public RoutePackage, public std::enable_shared_from_this<LoadSignalPackage> {
    // Read SPM
public:
    LoadSignalPackage(int dst_pe_row, int dst_pe_col, int dst_reg_idx, uint32_t addr, int interval, std::shared_ptr<Inst> inst);
    void operate(std::shared_ptr<Router> router) ; 
    int dst_pe_row_idx;
    int dst_pe_col_idx;
    int reg_idx;
...
class StoreDataPackage final : public RoutePackage, public std::enable_shared_from_this<StoreDataPackage>{
    // Write to spm
public:
    StoreDataPackage(int src_pe_row, int src_pe_col, uint32_t addr, VectorData src_data, int interval, std::shared_ptr<Inst> inst);
    void operate(std::shared_ptr<Router> router); 
    int pe_row, pe_col;
    uint32_t spm_addr;
    int interval;
